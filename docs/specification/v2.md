...
graph TD
    ...
    subgraph Browser/External Interfaces
        BrowserAPI[Chrome APIs (Messaging, Storage, etc.)]
        Storage[chrome.storage (Local Settings)]
        WebPage[Web Page (DOM)]
        Backend[Backend API (Server)]
    end

    %% Core Logic and Communication Flow via Browser APIs
    CS -- "1. Sends Sentences/Events" --> BrowserAPI;
    BrowserAPI -- "2. Relays Message" --> BSW;
    BSW -- "3. Processes/Calls Backend" --> Backend;
    Backend -- "4. Returns Concepts/Results" --> BSW;
    BSW -- "5. Sends Augmentations/Commands" --> BrowserAPI;
    BrowserAPI -- "6. Relays Message" --> CS;
    CS -- "7. Modifies DOM" --> WebPage;
    CS -- "Reads DOM" --> WebPage;

    %% UI Interactions
    Popup -- "User Actions (Toggle, Mode)" --> BrowserAPI;
    BrowserAPI -- "Sends Commands" --> BSW;
    BSW -- "Sends Status" --> BrowserAPI;
    BrowserAPI -- "Updates Popup" --> Popup;

    Options -- "User Configures Settings" --> BrowserAPI;
    BrowserAPI -- "Writes Settings" --> Storage;
    Options -- "Reads Settings" --> BrowserAPI;
    BrowserAPI -- "Reads Settings" --> Storage;
    Popup -- "Links to" --> Options;

    %% Storage Interaction
    BSW -- "Reads Settings/State" --> Storage;
    BSW -- "Writes State (Optional)" --> Storage;
