...
graph TD
    ...
    BSW --- BrowserAPI --- CS
    BSW --- BrowserAPI --- Popup
    BSW --- BrowserAPI --- Options
    BSW --- Backend
    BSW --- Storage
    CS --- WebPage
    Options --- Storage
...
graph TD
    subgraph Augmentation Process
        CS["Content Script"] -- "1. Extracts Sentences" --> BAPI1["Browser Messaging API"]
        BAPI1 -- "2. Sends to Background" --> BSW["Background Service Worker"]
        BSW -- "3. Requests Augmentation" --> Backend["Backend API"]
        Backend -- "4. Returns Augmented Text" --> BSW
        BSW -- "5. Sends Results" --> BAPI2["Browser Messaging API"]
        BAPI2 -- "6. Relays to Content Script" --> CS
        CS -- "7. Updates DOM Safely" --> DOM["Web Page DOM"]
    end
